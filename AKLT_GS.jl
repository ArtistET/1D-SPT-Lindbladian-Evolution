...
function generate_mps_path( N, t1, t2, tR, tD, J, U, Dmax, Dstep)
    ...
    mps_path="./ground_states/N$(N)_t($(t1),$(t2))_tR$(tR)_tD$(tD)_J$(J)_U$(U)/Dmax$(Dmax)/Dstep$(Dstep)/AKLT_N$(N)_t($(t1),$(t2))_tR$(tR)_tD$(tD)_J$(J)_U$(U)_Dmax$(Dmax).jld2"
    return mps_path
end
...
function dmrg_GS( N, H, mps_path, psi0, initD, Dstep, Dmax; eps=1e-10)
    ...
    for n = 1:400
        ...
        # EPS        = min(max(eps, abs(E_now*eps/N)),0.5)  #Care for the eps setting and adjust it immediately when things goes wrong.
        EPS        = eps   #According to log, E_diff should be less than 1e-8
