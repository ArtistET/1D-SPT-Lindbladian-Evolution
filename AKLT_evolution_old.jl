...
function measure_SO_for_rho_apply(SO_head,SO_body,SO_tail,IN,rho_t)
...
function measure_SO_for_rho(SO_mpo,IN,rho_t)
    C_value = -inner(SO_mpo, rho_t)/inner(IN, rho_t)
    SO_value = real(C_value)
    return C_value, SO_value
end
...
function Lindblad_evolution(dt,tsmax,init_t, N, t1, t2, tR, tD, J, U, I1, I2, IR, ID, Dmax,rho,K0,K0_dag,K_list,Kdag_list,SO_mpo_odd,SO_mpo_even,IN;block_size=4,cutoff=1e-8)
    for i = 1:tsmax
        t_total = time()
        t_save = time()
        slice_path_t = generate_slice_path(init_t+dt*(i-1), N, t1, t2, tR, tD, J, U, I1, I2, IR, ID, Dmax)
        save_slice(rho, slice_path_t)
        save_cost = time()-t_save
        t_measure = time()
        C_odd, SOV_odd = measure_SO_for_rho(SO_mpo_odd,IN,rho)
        C_even, SOV_even = measure_SO_for_rho(SO_mpo_even,IN,rho)
        measure_cost = time()-t_measure
        ...
        evolve_cost = time()-t_evolve
        println("Step ", i, "/", tsmax, " (T ", round(init_t+dt*(i-1), digits=6), " -> ", round(init_t+dt*i, digits=6), ") total=", format_hms(time()-t_total), " save=", format_hms(save_cost), " measure=", format_hms(measure_cost), " evolve=", format_hms(evolve_cost), " (hh:mm:ss), maxlinkdim(rho)= ", maxlinkdim(rho))
    end
    t_final = time()
    t_save = time()
    ...
    println("complex SO_odd= ", C_odd, "  SO_odd= ", SOV_odd)
    println("complex SO_even= ", C_even, "  SO_even= ", SOV_even)
    println("Final output total=", format_hms(time()-t_final), " save=", format_hms(save_cost), " measure=", format_hms(measure_cost), " (hh:mm:ss)")
    return nothing
end
...
function main()
    ...
    SO_2_odd, _ = measure_SO_for_rho(SO_mpo_odd,IN,rho0)
    SO_2_even, _ = measure_SO_for_rho(SO_mpo_even,IN,rho0)
    println("Direct MPO–MPO SO_odd = ", SO_2_odd, " SO_even= ", SO_2_even,
        " differences=", (abs(SO_2_odd-SO_1_odd), abs(SO_2_even-SO_1_even)))
    isapprox(SO_2_odd,SO_1_odd;rtol=1e-6,atol=1e-9) || error("Odd MPO–MPO SO measurement disagrees with apply method")
    isapprox(SO_2_even,SO_1_even;rtol=1e-6,atol=1e-9) || error("Even MPO–MPO SO measurement disagrees with apply method")
    # ======================================================================
    ...
    Lindblad_evolution(dt,tsmax,init_t,N, t1, t2, tR, tD, J, U, I1, I2, IR, ID, Dmax,rho0,K0,K0_dag,K_list,Kdag_list,SO_mpo_odd,SO_mpo_even,IN)
end
