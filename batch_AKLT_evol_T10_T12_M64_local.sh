...
cd "$PROJECT_DIR"
mkdir -p log
manifest="log/trajectory_T10_T12_M64_local_$(date +%Y%m%d_%H%M%S).log"
...
do
    ...
    do
        result="$run_dir/results/T10.0_to_T12.0_traj${trajectory_id}-${trajectory_id}.jld2"
...
wait
printf 'all T=10->12 trajectories finished\n' | tee -a "$manifest"
